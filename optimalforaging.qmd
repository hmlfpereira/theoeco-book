...
![A pack of wolfs (*Canis lupus*) roaming around the landscape of Peneda-Gerês National Park, Portugal.](images/Canis_lupus.jpg){#fig-Canis_lupus}
...
Now, we can output the result of the integral from @eq-total-abundance in a beautiful way as[^optimalforaging-1],

[^optimalforaging-1]: Note that this only displays the typeset equation when you render the Quarto document into html or PDF, and it is not shown directly on the notebook output mode. You also need to add the command "#\| output: asis" in the beginning of the R chunk to tell Quarto it needs to take the output literally.
...
The waiting time is just the inverse of the total abundance, $$t_w=1/A$$ or in R code,
...
where we highlighted that the wating time and the pursuit time are functions of the territory size or cut-off radius $r_c$. Let's calculate this expression in R,
...
Let's plot the energy per time as a function of the territory size. We have to start by giving some values to the different paramerters.
...
Next we convert the yacas expresssion for ET into a R expression using `yac_expr()` and then write a function that evaluates that R expression for a given value of rc,
...
$$
 \frac{\mathrm{d}ET}{\mathrm{d}r_c}=0
$$
