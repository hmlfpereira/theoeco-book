...
This is an easy integral, but let's see how we could solve it in R, as this will be handy for the next one. We can do symbolic math in R using the package Ryacas. We just say that *r* is a symbolic variable um the function ysym() and then integrate the product $a \ \pi \ r$ from 0 to $r_c$ with the function integrate(),
...
Or more beatifully, using the TeX ouput,

```{r}
#| output: asis
tex_quarto = function (x)
  cat("$$", tex(x), "$$")
tex_quarto(A)
```
