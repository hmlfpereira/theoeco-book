...
The general solution that both Maynard Smith and Nash came up for these games is that one needs to identify which strategy is the best response to itself. This is called an Evolutionary Stable Strategy (ESS, also known as Nash Equilibrium[^gametheory-1]) because it is the strategy that when dominant in the population cannot be invaded by any other strategy. Mathematically speaking, the ESS is the strategy $S^*$ that has the higher payoff when played against itself,
